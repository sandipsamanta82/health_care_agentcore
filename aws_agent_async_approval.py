...
def human_approval_node(state: AgentState):
    """Save the approval request to database and pause"""
    last_message = state["messages"][-1]
    
    if last_message.tool_calls:
        tool_call = last_message.tool_calls[0]
        
        # Extract thread_id from state metadata (you'll need to pass this in config)
        # For now, we'll use a default or extract from somewhere
        thread_id = "default_thread"  # This should come from config
...
def submit_payment_request(user_message: str, thread_id: str = "thread_1"):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print('  Step 1 - Submit request: python aws_agent_async_approval.py submit "Send $500 to Alice"')
        print("  Step 2 - Process decision: python aws_agent_async_approval.py process <approval_id>")
        print("\nExamples:")
        print('  python aws_agent_async_approval.py submit "Send $500 to Alice"')
        print('  python aws_agent_async_approval.py submit "Transfer 1000 dollars to Bob"')
        print('  python aws_agent_async_approval.py submit "Pay Alice $250"')
        sys.exit(1)
    ...
    if command == "submit":
        if len(sys.argv) < 3:
            print('Usage: python aws_agent_async_approval.py submit "Your natural language request"')
            print('Example: python aws_agent_async_approval.py submit "Send $500 to Alice"')
            sys.exit(1)
        # Join all remaining arguments as the message
        user_message = " ".join(sys.argv[2:])
        submit_payment_request(user_message)
        
    elif command == "process":
        if len(sys.argv) < 3:
            print("Usage: python aws_agent_async_approval.py process <approval_id>")
            sys.exit(1)
